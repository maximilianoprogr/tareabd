...
if __name__ == "__main__":
    # Generar datos
    usuarios = generate_usuarios(100)  # Generar al menos 100 usuarios para garantizar 50 autores y 50 revisores
    autores = generate_autores(usuarios)
    revisores = generate_revisores(usuarios)

    # Asegurar que haya al menos 50 autores y 50 revisores
    while len(autores) < 50 or len(revisores) < 50:
        usuarios += generate_usuarios(10)  # Generar más usuarios si no se cumple la cantidad mínima
        autores = generate_autores(usuarios)
        revisores = generate_revisores(usuarios)

    articulos = generate_articulos(autores, 300)  # Generar 300 artículos
    topicos = generate_topicos(20)  # Generar 20 tópicos
